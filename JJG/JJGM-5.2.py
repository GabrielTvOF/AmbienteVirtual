...
pygame.display.set_caption("Mapa de Masmorra com Sala de Baú")
...
def desenhar_mapa(mapa, inicio, fim, jogador_pos, bau_pos):
...
def main():
    ...
    while running:
        for event in pygame.event.get():
            ...
            # Processamento das teclas pressionadas uma vez por evento
                ...
                if 0 <= nova_posicao[0] < LARGURA_GRADE and 0 <= nova_posicao[1] < ALTURA_GRADE:
                    if mapa[nova_posicao[1]][nova_posicao[0]] == 0 or nova_posicao == bau_pos:  # Verifica se é um caminho ou a posição do baú
        ...
        desenhar_mapa(mapa, inicio, fim, jogador_pos, bau_pos if not bau_coletado else None)  # Desenha o mapa e o jogador
