        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:  # Pressionando Enter para confirmar
                    digitando = False
                elif event.key == pygame.K_BACKSPACE:  # Deletando o último caractere
                    nome = nome[:-1]
                else:
                    nome += event.unicode  # Adiciona o caractere à string

        # Exibe o campo de texto
        screen.fill(COR_MENU)
        texto = font_menu.render("Digite seu nome:", True, COR_TEXTO)
        screen.blit(texto, (LARGURA // 2 - texto.get_width() // 2, ALTURA // 2 - 100))
        texto_nome = font_menu.render(nome, True, COR_TEXTO)
        screen.blit(texto_nome, (LARGURA // 2 - texto_nome.get_width() // 2, ALTURA // 2))
        pygame.display.flip()
