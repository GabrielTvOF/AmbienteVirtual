...
pygame.display.set_caption("Masmorra com Itens Rápidos")
...
def desenhar_mapa(mapa, fim, jogador_pos, bau_pos, pontuacao):
...
def salvar_jogo(jogador_pos, pontuacao, mapa, bau_pos, tempo_inicio):
    with open('savegame.pkl', 'wb') as f:
        pickle.dump((jogador_pos, pontuacao, mapa, bau_pos, tempo_inicio), f)

# Função de carregar o jogo
def carregar_jogo():
    try:
        with open('savegame.pkl', 'rb') as f:
            jogador_pos, pontuacao, mapa, bau_pos, tempo_inicio = pickle.load(f)
            return jogador_pos, pontuacao, mapa, bau_pos, tempo_inicio
...
def main():
    ...
    while running:
            ...
            # Processamento de teclas
                ...
                if not em_menu:
                    ...
                    if event.key == pygame.K_UP:
                        ...
                        if opcao_selecionada == 0:  # Salvar Jogo
                            salvar_jogo(jogador_pos, pontuacao, mapa, bau_pos, tempo_inicio)
                            print("Jogo salvo.")
                        elif opcao_selecionada == 1:  # Carregar Jogo
                            jogador_pos, pontuacao, mapa, bau_pos, tempo_inicio = carregar_jogo()
        ...
        if mensagem_erro:
            desenhar_menu(["Salvar Jogo", "Carregar Jogo"], opcao_selecionada, mensagem_erro)
            ...
            if not em_menu:
                ...
                desenhar_menu(["Salvar Jogo", "Carregar Jogo"], opcao_selecionada)
