...
def main():
    nome = pedir_nome()  # Pede o nome do jogador no início do jogo
            ...
            # Processamento de teclas
                ...
                if not em_menu:
                    ...
                    if jogador_pos == fim:
                        ...
                        atualizar_ranking(nome, pontuacao)  # Atualiza o ranking com o nome e pontuação
                        mapa, inicio, fim, bau_pos = gerar_mapa()
                        jogador_pos = inicio
                        bau_coletado = False
                    ...
                    if event.key == pygame.K_UP:
                        ...
                        if opcao_selecionada == 0:  # Ranking
                            ranking = mostrar_ranking()
                            mostrar_ranking_visual(ranking)
...
# Função para exibir o ranking visualmente
def mostrar_ranking_visual(ranking):
    screen.fill(COR_MENU)
    y_offset = 50
    for i, (nome, pontos) in enumerate(ranking[:10]):  # Limita a exibição dos 10 primeiros jogadores
        texto = font_ranking.render(f"{i + 1}. {nome}: {pontos} pontos", True, COR_TEXTO)
        screen.blit(texto, (LARGURA // 2 - texto.get_width() // 2, y_offset))
        y_offset += 40
    
    pygame.display.flip()
    pygame.time.wait(3000)  # Exibe o ranking por 3 segundos antes de voltar ao menu
