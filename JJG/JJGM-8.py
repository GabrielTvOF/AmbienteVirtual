...
def desenhar_menu(opcoes, opcao_selecionada):
...
def salvar_jogo(jogador_pos, pontuacao):
    with open('savegame.pkl', 'wb') as f:
        pickle.dump((jogador_pos, pontuacao), f)

# Função de carregar o jogo
def carregar_jogo():
    try:
        with open('savegame.pkl', 'rb') as f:
            jogador_pos, pontuacao = pickle.load(f)
            return jogador_pos, pontuacao
                ...
                if not em_menu:
                    ...
                    if event.key == pygame.K_UP:
                        ...
                        if opcao_selecionada == 0:  # Inventário
                            ...
                            salvar_jogo(jogador_pos, pontuacao)
                            print("Jogo salvo.")
                        em_menu = False  # Fecha o menu após selecionar
                        
        if not em_menu:
            screen.fill((0, 0, 0))
            desenhar_mapa(mapa, inicio, fim, jogador_pos, bau_pos if not bau_coletado else None, pontuacao)
        else:
            desenhar_menu(["Inventário", "Loja", "Salvar Jogo"], opcao_selecionada)
