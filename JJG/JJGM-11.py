...
def salvar_jogo(jogador_pos, pontuacao, mapa, bau_pos, inventario):
    with open('savegame.pkl', 'wb') as f:
        pickle.dump((jogador_pos, pontuacao, mapa, bau_pos, inventario), f)

# Função de carregar o jogo
def carregar_jogo():
    try:
        with open('savegame.pkl', 'rb') as f:
            jogador_pos, pontuacao, mapa, bau_pos, inventario = pickle.load(f)
            return jogador_pos, pontuacao, mapa, bau_pos, inventario
                ...
                if not em_menu:
                    ...
                    if event.key == pygame.K_UP:
                        ...
                        if opcao_selecionada == 0:  # Inventário
                            ...
                            if len(loja_itens) > 0:
                                item_comprado = loja_itens.pop(0)
                                inventario.append(item_comprado)
                                pontuacao -= 10
                        elif opcao_selecionada == 2:  # Salvar Jogo
                            salvar_jogo(jogador_pos, pontuacao, mapa, bau_pos, inventario)
                            print("Jogo salvo.")
                        elif opcao_selecionada == 3:  # Carregar Jogo
                            jogador_pos, pontuacao, mapa, bau_pos, inventario = carregar_jogo()
