...
pygame.display.set_caption("Mapa de Masmorra com Salas, Corredores e Baú")
...
def gerar_mapa():
    ...
    # Escolher uma sala aleatória para ser a sala do baú, diferente da inicial e final
    sala_bau = random.choice(salas[1:-1])  # Evita o início e o fim
    return mapa, salas[0], salas[-1], sala_bau  # Retorna o mapa, início, fim e sala do baú

# Função para desenhar o mapa na tela
def desenhar_mapa(mapa, inicio, fim, jogador_pos, sala_bau):
...
def main():
    mapa, inicio, fim, sala_bau = gerar_mapa()
    jogador_pos = inicio  # A posição inicial do jogador é a mesma que a posição de início
    encontrou_bau = False  # Flag para verificar se o jogador encontrou o baú
    ...
    while running:
        ...
        if jogador_pos == fim:
            ...
            encontrou_bau = False  # Reinicia o status do baú
        
        # Verifica se o jogador encontrou o baú
        if jogador_pos == sala_bau and not encontrou_bau:
            encontrou_bau = True  # Marca o baú como encontrado
        
        screen.fill((0, 0, 0))  # Limpa a tela com a cor preta
        desenhar_mapa(mapa, inicio, fim, jogador_pos, sala_bau)  # Desenha o mapa e o jogador
        
        # Exibe mensagem se o baú foi encontrado
        if encontrou_bau:
            texto = fonte.render("Você encontrou o baú!", True, (255, 255, 255))
            screen.blit(texto, (LARGURA // 2 - texto.get_width() // 2, ALTURA - 40))
